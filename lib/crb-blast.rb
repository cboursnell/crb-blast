...
class CRB_Blast
  ...
  def makedb
    ...
    @target_is_prot = entry.isProt?
    # construct the output database names
    @query_name = File.basename(opts.query).split('.')[0..-2].join('.')
    @target_name = File.basename(opts.target).split('.')[0..-2].join('.')
